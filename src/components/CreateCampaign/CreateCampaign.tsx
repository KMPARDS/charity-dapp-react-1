...
function Step4(props : inputProps){
  ...
  const [byte, setByte] = useState('loading...');
    ...
    const handleSubmit = async ()=>{
      ...
      if(window.wallet){
        const A = await window.charityInstance.connect(window.wallet).populateTransaction.newCampaign(Hash,mapState.Benificery,ethers.utils.parseEther(mapState.RaisedFund),mapState.fullExtraction,renderTimeStamp(mapState.endTime));
        console.log("call : ",A); 
        Swal.fire({
          title: "Are you sure?", 
          ...
          preConfirm: () => {
            return window.charityInstance.connect(window.wallet).newCampaign(Hash,mapState.Benificery,ethers.utils.parseEther(mapState.RaisedFund),mapState.fullExtraction,renderTimeStamp(mapState.endTime))
            .then(async res => {
              const x = await res.wait(); 
              const event = await window.charityInstance.interface.parseLog(x.logs[0]);
              setHash(res.hash);setByte(event.args[0]);setDone(true);
              ...
              try {
                const A = await window.charityInstance.connect(window.wallet).estimateGas.newCampaign(Hash,mapState.Benificery,ethers.utils.parseEther(mapState.RaisedFund),mapState.fullExtraction,renderTimeStamp(mapState.endTime))
                console.log(A);
              } catch (e) {
    ...
    const TxnDetails = ({ hash, byte }) => {
      return (
        <>
          <br />
          <i className="fa fa-check-circle text-center" style={{ fontSize: '8em', color: '#54BE3D' }}></i>
        ...
        <div className="row mt40">
            ...
            <Card className="shadow">
                ...
                <Form>
                    ...
                    <Button className="mb10 ml10 btn btn-dark" onClick={handleSubmit} variant="dark">Submit</Button>
